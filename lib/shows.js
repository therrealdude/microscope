...
Meteor.methods({
    ...
    showUpdate: function(showId, show, imagesToDelete) {
        Shows.update(showId, {$set: show}, function(error, result){
            if (error){
	...
	searchShows: function(searchCriteria){
		if(Meteor.isServer){
			...
			if(includeKeywords && includeDates && includeLocation && includeTags){
				...
				venue: {$in: venue_ids},
                tags: {$in: searchCriteria.tags}}).map(function(s){
					var distance = 0;
            ...
            else if(includeKeywords && includeDates && includeLocation){
				return Shows.find({name: {$regex: new RegExp(searchCriteria.keywords, "i")},
				'dates.0.date': {$gte: new Date(searchCriteria.startdate), $lte: new Date(searchCriteria.enddate) },
            ...
            else if(includeKeywords && includeDates && includeTags){
				return Shows.find({name: {$regex: new RegExp(searchCriteria.keywords, "i")},
				'dates.0.date': {$gte: new Date(searchCriteria.startdate), $lte: new Date(searchCriteria.enddate) },
                tags: {$in: searchCriteria.tags}}).fetch();
			}
            else if(includeKeywords && includeLocation && includeTags){
				return Shows.find({name: {$regex: new RegExp(searchCriteria.keywords, "i")},
				venue: {$in: venue_ids},
                tags: {$in: searchCriteria.tags}}).map(function(s){
					var distance = 0;
            ...
            else if(includeDates && includeLocation && includeTags){
            ...
            else if(includeKeywords && includeTags){
				return Shows.find({name: {$regex: new RegExp(searchCriteria.keywords, "i")},
				'dates.0.date': {$gte: new Date(searchCriteria.startdate), $lte: new Date(searchCriteria.enddate) },
                tags: {$in: searchCriteria.tags}}).fetch();
			}
            else if(includeDates && includeTags){
				return Shows.find({'dates.0.date': {$gte: new Date(searchCriteria.startdate), $lte: new Date(searchCriteria.enddate) },
                tags: {$in: searchCriteria.tags}}).fetch();
			}
            else if(includeLocation && includeTags){
				return Shows.find({venue: {$in: venue_ids},
                tags: {$in: searchCriteria.tags}}).map(function(s){
					var distance = 0;
					for(var i = 0; i < venues.length; i++){
						if(venues[i].venue_id === s.venue){
				...
				return Shows.find({tags: {$in: searchCriteria.tags}});
			}
            else{
